...
export default function CheckoutPage() {
  ...
  const totalAmount = subTotal - discountAmount + (cart.length > 0 ? shippingFee : 0); 
  ...
  const handlePlaceOrder = async (e) => {
    ...
    if (cart.length === 0) {
      alert("Giỏ hàng trống trơn kìa ní ơi! Mua gì đi rồi hẵng chốt đơn!");
      router.push("/");
      return;
    }

    if (!formData.fullName || !formData.phone || !formData.address) {
      alert("Điền thiếu thông tin giao hàng rồi Chủ Tịch ơi!");
      return;
    }
    ...
    try {
      ...
      // ==========================================
      // 🚀 BƯỚC 1: BẺ KHÓA TOKEN ĐỂ LẤY ID THẬT CỦA USER MỘT CÁCH TÀN BẠO
      // ==========================================
      ...
      if (!realUserId && currentToken) {
        try {
          ...
          console.log("🛠️ Đã dùng tuyệt kỹ lôi được ID ra từ Token:", realUserId);
        } catch (error) {
      ...
      if (!realUserId) {
         realUserId = "64a5462f086d74c9e772b804"; // ID ảo cứu cánh cuối cùng
      }

      // ==========================================
      // 🚀 BƯỚC 2: RẢI THẢM DỮ LIỆU ĐỂ BACKEND HẾT ĐƯỜNG TỪ CHỐI
      // ==========================================
      const orderPayload = {
        // Ném ID đủ mọi định dạng
        user: realUserId, 
        userId: realUserId,
        
        // Ném danh sách hàng hóa
        items: orderItemsData,       
        orderItems: orderItemsData,  
        
        // Ném Tên, SĐT, Địa chỉ NẰM TRẦN BÊN NGOÀI (Chống vụ Backend bỏ qua shippingAddress)
        fullName: formData.fullName,
        name: formData.fullName,
        phone: formData.phone,
        address: formData.address,

        // Vẫn gửi kèm cái chuẩn shippingAddress phòng hờ
        shippingAddress: {
          fullName: formData.fullName,
          name: formData.fullName,
          phone: formData.phone,
      ...
      console.log("📦 CỤC DATA CHUẨN BỊ BAY LÊN BACKEND:", orderPayload);
      ...
      alert("🎉 CHỐT ĐƠN THÀNH CÔNG! Hàng sẽ về bản trong vài ngày tới!");
      ...
      alert(`❌ Lỗi rồi Chủ Tịch ơi! Backend nó chê nè:\n\n👉 "${backendError}"`);
    } finally {
      setLoading(false);
    }
  };
                ...
                <div className="border-t border-slate-100 pt-6 space-y-4">
                  ...
                  {discountPercent > 0 && (
                    <div className="flex justify-between text-green-600 font-black text-sm bg-green-50 px-4 py-2.5 rounded-xl border border-green-100">
                      <span className="flex items-center gap-1.5"><span className="text-lg">🎟️</span> Voucher ({discountPercent}%)</span>
